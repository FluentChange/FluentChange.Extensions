...
namespace FluentChange.Extensions.Azure.Functions.CRUDL
{
    public class ResponseBuilderIsolated
    {
        ...
        {
            // Reset ID for each request to avoid state leaking between requests
            this.id = null;

            // Extract ID from routeData if present
            if (routeData.ContainsKey("id") && routeData["id"] != null)
            {
                var idStr = routeData["id"]!.ToString();
                if (!string.IsNullOrEmpty(idStr) && Guid.TryParse(idStr, out var idGuid))
                {
                    this.id = idGuid;
                }
            }
        ...
        {
            var builder = new ResponseBuilderWithIdEntityIsolated<T, T>(provider, contextCreateFunc, jsonOptions, id);
            return builder;
        }

        public ResponseBuilderWithIdEntityIsolated<T, M> ForEntityWithMapping<T, M>() where T : class where M : class
        {
            var builder = new ResponseBuilderWithIdEntityIsolated<T, M>(provider, contextCreateFunc, jsonOptions, id);
            return builder;
        }
        ...
        public ResponseBuilderWithIdEntityServiceIsolated<TServiceModel> Use<TServiceModel>() where TServiceModel : class
        {
            ...
            var builder = new ResponseBuilderWithIdEntityServiceIsolated<TServiceModel>(service, contextCreateFunc, mapper, jsonOptions, id);
            return builder;
        }
    ...
    public class ResponseBuilderWithIdEntityIsolated<T, M> where T : class where M : class
    {
        ...
        public ResponseBuilderWithIdEntityServiceWithModelsIsolated<T, M, S> Use<S>() where S : class
        {
            ...
            var builder = new ResponseBuilderWithIdEntityServiceWithModelsIsolated<T, M, S>(service, contextCreateFunc, mapper, jsonOptions, id);
            return builder;
        }
        ...
        public ResponseBuilderWithIdEntityInterfaceServiceIsolated<T, M, S> UseInterface<S>() where S : class, ICRUDLServiceWithId<T>
        {
            ...
            var builder = new ResponseBuilderWithIdEntityInterfaceServiceIsolated<T, M, S>(service, contextCreateFunc, mapper, jsonOptions, id);
            return builder;
        }
        private IEntityMapper GetMapperService()
        {
    ...
    public class ResponseBuilderWithIdEntityInterfaceServiceIsolated<T, M, S> where S : class, ICRUDLServiceWithId<T> where M : class where T : class
    {
        ...
        public ResponseBuilderWithIdEntityInterfaceServiceIsolated(S service, Func<ILogger, Task>? contextCreateFunc, IEntityMapper mapper, JsonSerializerOptions? jsonOptions, Guid? id)
        {
            if (service == null) throw new ArgumentNullException(nameof(service));
            if (mapper == null) throw new ArgumentNullException(nameof(mapper));
            this.contextCreateFunc = contextCreateFunc;

            internalBuilder = new ResponseBuilderWithIdEntityServiceWithModelsIsolated<T, M, S>(service, contextCreateFunc, mapper, jsonOptions, id);
    ...
    public class ResponseBuilderWithIdEntityServiceIsolated<TService> : AbstractResponseHelpersIsolated where TService : class
    {
        ...
        public ResponseBuilderWithIdEntityServiceIsolated(TService service, Func<ILogger, Task>? contextCreateFunc, IEntityMapper mapper, JsonSerializerOptions? jsonOptions, Guid? id) : base(mapper, jsonOptions)
        {
            if (service == null) throw new ArgumentNullException(nameof(service));
            if (mapper == null) throw new ArgumentNullException(nameof(mapper));
        ...
        // async, no need for mapping outgoing
        public ResponseBuilderWithIdEntityServiceIsolated<TService> OnGet<OutgoingModel>(Func<TService, Func<Task<OutgoingModel>>> predicate)
            where OutgoingModel : class
        {
            MakeGetFuncAsync<OutgoingModel, OutgoingModel>(predicate);
            return this;
        }
        ...
        #region POST
        // no body, only model outgoing (sync)
        ...
        {
            MakePostFuncNoBody<Model, Model>(predicate);
            return this;
        }
        // no body, only model outgoing (async)
        public ResponseBuilderWithIdEntityServiceIsolated<TService> OnPost<Model>(Func<TService, Func<Task<Model>>> predicate) where Model : class
        {
        ...
        protected void MakeGetFuncAsync<OutgoingServiceModel, OutgoingModel>(Func<TService, Func<Task<OutgoingServiceModel>>> predicate)
            where OutgoingServiceModel : class
            where OutgoingModel : class
        {
            getFunc = async (TService service, HttpRequestData req) =>
            {
                var result = await predicate.Invoke(service).Invoke();
                return Respond<OutgoingServiceModel, OutgoingModel>(req, result, wrapout);
            };
        }
        ...
        protected void MakePostFuncNoBody<OutgoingServiceModel, OutgoingModel>(Func<TService, Func<OutgoingServiceModel>> predicate)
            where OutgoingServiceModel : class
            where OutgoingModel : class
        {
        ...
        protected void MakePostFuncNoBodyAsync<OutgoingServiceModel, OutgoingModel>(Func<TService, Func<Task<OutgoingServiceModel>>> predicate)
            where OutgoingServiceModel : class
            where OutgoingModel : class
        {
            postFunc = async (TService service, HttpRequestData req) =>
        ...
        protected void MakePutFuncAsync<IngoingModel, IngoingServiceModel, OutgoingServiceModel, OutgoingModel>(Func<TService, Func<IngoingServiceModel, Task<OutgoingServiceModel>>> predicate)
         where IngoingModel : class
         where IngoingServiceModel : class
         where OutgoingServiceModel : class
         where OutgoingModel : class
        {
            putFunc = async (TService service, HttpRequestData req) =>
            {
                ...
                if (updateData == null) throw new ArgumentNullException();
                var resultUpdated = await predicate.Invoke(service).Invoke(updateData);
        ...
        public async Task<HttpResponseData> Handle(HttpRequestData req, ILogger log)
        {
            ...
            try
            {
                // GET: prefer WithId handler when id is present, fallback to regular handler
                if (req.Method == "GET")
                {
                    if (id.HasValue && getWithIdFunc != null)
                    {
                ...
                // POST: prefer WithId handler when id is present, fallback to regular handler
                if (req.Method == "POST")
                {
                    if (id.HasValue && postWithIdFunc != null)
                    {
                ...
                // PUT: prefer WithId handler when id is present, fallback to regular handler
                if (req.Method == "PUT")
                {
                    if (id.HasValue && putWithIdFunc != null)
                    {
                ...
                // DELETE: prefer WithId handler when id is present, fallback to regular handler
                if (req.Method == "DELETE")
                {
                    if (id.HasValue && deleteWithIdFunc != null)
                    {
    ...
    {
        public ResponseBuilderWithIdEntityServiceWithModelsIsolated(TService service, Func<ILogger, Task>? contextCreateFunc, IEntityMapper mapper, JsonSerializerOptions? jsonOptions, Guid? id) : base(service, contextCreateFunc, mapper, jsonOptions, id)
        {
        }
...
#nullable enable annotations
        ...
        public ResponseBuilderWithIdEntityServiceWithModelsIsolated<ServiceModel, ApiModel, TService> OnPut(Func<TService, Func<ServiceModel, Task<ServiceModel>>> predicate)
        {
            MakePutFuncAsync<ApiModel, ServiceModel, ServiceModel, ApiModel>(predicate);
            return this;
        }
