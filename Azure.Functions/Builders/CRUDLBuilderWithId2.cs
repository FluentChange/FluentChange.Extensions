...
{
    public class CRUDLBuilderWithId2
    {
        private readonly IServiceProvider provider;
        public CRUDLBuilderWithId2(IServiceProvider provider)
        {
            this.provider = provider;
        }

        public CRUDLBuilderWithIdEntity2<T, T> ForEntity<T>() where T : class
        {
            var builder = new CRUDLBuilderWithIdEntity2<T, T>(provider);
            return builder;
        }
        public CRUDLBuilderWithIdEntity2<T, M> ForEntityWithMapping<T, M>() where T : class where M : class
        {
            var builder = new CRUDLBuilderWithIdEntity2<T, M>(provider);
            return builder;
        }
        ...
        public CRUDLBuilderWithIdEntityService2<T, T, S> With<T, S>(Func<S, Func<T, T>> create, Func<S, Func<Guid, T>> read, Func<S, Func<T, T>> update, Func<S, Action<Guid>> delete, Func<S, Func<IEnumerable<T>>> list) where T : class where S : class
        {
            return ForEntity<T>().Use<S>().With(create, read, update, delete, list);
        }
        public CRUDLBuilderWithIdEntityService2<T, M, S> WithAndMap<T, M, S>(Func<S, Func<T, T>> create, Func<S, Func<Guid, T>> read, Func<S, Func<T, T>> update, Func<S, Action<Guid>> delete, Func<S, Func<IEnumerable<T>>> list) where T : class where S : class where M : class
        {
    ...
    public class CRUDLBuilderWithIdEntity2<T, M> where T : class where M : class
    {
        private readonly IServiceProvider provider;
        private readonly bool usesMapping;

        public CRUDLBuilderWithIdEntity2(IServiceProvider provider)
        {
            this.provider = provider;
            this.usesMapping = !(typeof(T).Equals(typeof(M)));
        }

        public CRUDLBuilderWithIdEntityService2<T, M, S> Use<S>() where S : class
        {
            ...
            var builder = new CRUDLBuilderWithIdEntityService2<T, M, S>(service, mapper);
            return builder;
        }

        public CRUDLBuilderWithIdEntityInterfaceService2<T, M, S> UseInterface<S>() where S : class, ICRUDLServiceWithId<T>
        {
            var service = provider.GetService<S>();
            var mapper = GetMapperService();
            ...
            var builder = new CRUDLBuilderWithIdEntityInterfaceService2<T, M, S>(service, mapper);
            return builder;
        }
    ...
    public class CRUDLBuilderWithIdEntityInterfaceService2<T, M, S> where S : class, ICRUDLServiceWithId<T> where M : class where T : class
    {
        private readonly CRUDLBuilderWithIdEntityService2<T, M, S> internalBuilder;
        public CRUDLBuilderWithIdEntityInterfaceService2(S service, IEntityMapper mapper)
        {
            if (service == null) throw new ArgumentNullException(nameof(service));
            if (mapper == null) throw new ArgumentNullException(nameof(mapper));

            internalBuilder = new CRUDLBuilderWithIdEntityService2<T, M, S>(service, mapper);
    ...
    public class CRUDLBuilderWithIdEntityService2<T, M, S> : AbstractResponseHelpersWithHandleId<T, M> where S : class where T : class where M : class
    {
        private readonly S service;

        public CRUDLBuilderWithIdEntityService2(S service, IEntityMapper mapper) : base(mapper)
        {
            if (service == null) throw new ArgumentNullException(nameof(service));
            if (mapper == null) throw new ArgumentNullException(nameof(mapper));
        ...
        public CRUDLBuilderWithIdEntityService2<T, M, S> With(Func<S, Func<T, T>> create, Func<S, Func<Guid, T>> read, Func<S, Func<T, T>> update, Func<S, Action<Guid>> delete, Func<S, Func<IEnumerable<T>>> list)
        {
            createFunc = create;
            readFunc = read;
            updateFunc = update;
        ...
        public CRUDLBuilderWithIdEntityService2<T, M, S> Create(Func<S, Func<T, T>> predicate)
        {
            createFunc = predicate;
            return this;
        }
        public CRUDLBuilderWithIdEntityService2<T, M, S> Read(Func<S, Func<Guid, T>> predicate)
        {
            readFunc = predicate;
            return this;
        }
        public CRUDLBuilderWithIdEntityService2<T, M, S> Update(Func<S, Func<T, T>> predicate)
        {
            updateFunc = predicate;
            return this;
        }
        public CRUDLBuilderWithIdEntityService2<T, M, S> Delete(Func<S, Action<Guid>> predicate)
        {
            deleteFunc = predicate;
            return this;
        }
        public CRUDLBuilderWithIdEntityService2<T, M, S> List(Func<S, Func<IEnumerable<T>>> predicate)
        {
            listFunc = predicate;
            return this;
        }
